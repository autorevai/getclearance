...
def upgrade() -> None:
    ...
    if not index_exists('batch_jobs', 'idx_batch_jobs_tenant'):
        op.create_index(
            'idx_batch_jobs_tenant',
            'batch_jobs',
            ['tenant_id']
        )
    if not index_exists('batch_jobs', 'idx_batch_jobs_tenant_status'):
        op.create_index(
            'idx_batch_jobs_tenant_status',
            'batch_jobs',
            ['tenant_id', 'status']
        )
    if not index_exists('batch_jobs', 'idx_batch_jobs_tenant_type'):
        op.create_index(
...
def downgrade() -> None:
    if table_exists('batch_jobs'):
        # Drop indexes
        if index_exists('batch_jobs', 'idx_batch_jobs_tenant_type'):
            op.drop_index('idx_batch_jobs_tenant_type', table_name='batch_jobs')
        if index_exists('batch_jobs', 'idx_batch_jobs_tenant_status'):
            op.drop_index('idx_batch_jobs_tenant_status', table_name='batch_jobs')
        if index_exists('batch_jobs', 'idx_batch_jobs_tenant'):
