...
def upgrade() -> None:
    ...
    if not index_exists('tenant_settings', 'idx_tenant_settings_lookup'):
        op.create_index(
    ...
    if not index_exists('tenant_settings', 'idx_tenant_settings_tenant'):
        op.create_index(
    ...
    if not index_exists('team_invitations', 'idx_team_invitations_tenant_email'):
        op.create_index(
            'idx_team_invitations_tenant_email',
            'team_invitations',
            ['tenant_id', 'email']
        )
    if not index_exists('team_invitations', 'idx_team_invitations_tenant'):
        op.create_index(
            'idx_team_invitations_tenant',
            'team_invitations',
            ['tenant_id']
        )
    if not index_exists('team_invitations', 'idx_team_invitations_status'):
        op.create_index(
...
def downgrade() -> None:
    ...
    if table_exists('team_invitations'):
        if index_exists('team_invitations', 'idx_team_invitations_status'):
            op.drop_index('idx_team_invitations_status', table_name='team_invitations')
        if index_exists('team_invitations', 'idx_team_invitations_tenant'):
            op.drop_index('idx_team_invitations_tenant', table_name='team_invitations')
        if index_exists('team_invitations', 'idx_team_invitations_tenant_email'):
    ...
    if table_exists('tenant_settings'):
        if index_exists('tenant_settings', 'idx_tenant_settings_tenant'):
            op.drop_index('idx_tenant_settings_tenant', table_name='tenant_settings')
        if index_exists('tenant_settings', 'idx_tenant_settings_lookup'):
